...
from DBAction import GetPosts, FindUser, GetWarning
from Appeal import AppealWindow
...
class ForumWindow(QMainWindow):
    def __init__(self, page, currForum, userID, widget, FrontPageUI):
        ...
        if userID != False:
            ...
            self.username = 0
            self.loginas.setText("Not logged in!")
            self.NewPostBtn.setEnabled(False)
        if self.naughty:
            self.loginas.setText(self.loginas.text()+"\nYou're unable to\nparticipate due to\nbeing naughty\nred handed!")
            self.NewPostBtn.setEnabled(False)
            self.AppealBtn.show()
            if FindUser(userID)[7] == 1:
                self.AppealBtn.setEnabled(False)
            else:
                self.AppealBtn.clicked.connect(self.Appeal)
    ...
    def Appeal(self):
        ...
        AppealUI = AppealWindow(self.userID)
        AppealUI.GoBackBtn.clicked.connect(lambda:
            self.GoToWindow(ForumWindow(page, self.currForum, self.userID, self.widget, self.FrontPageUI)))
        AppealUI.SubmitBtn.clicked.connect(lambda:
            self.GoToWindow(ForumWindow(page, self.currForum, self.userID, self.widget, self.FrontPageUI)))
        self.GoToWindow(AppealUI)
    ...
    def LoadPosts(self, nextOrprevious, GoTo):
        ...
        if int(self.Current_page.text()[14:]) == 1 and nextOrprevious == -1:
            ...
            POSTS = GetPosts(self.currForum, startIndex, postPerPage)
            if POSTS == [] and startIndex != 0:
                self.LoadPosts(0, page)
            else:
                for post in POSTS:
                    title = post[1]
                    if len(title) > 50:
...
if __name__ == "__main__":
    ...
    ForumUI = ForumWindow(1, "GPU", 55555, widget, None)
